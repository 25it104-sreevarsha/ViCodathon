...
Still not implemented (out of scope per the task brief): frontend,
authentication, voice/video, a database, MCP server, final deployment.
...
npm run test:memory  # Part 3 — Breeth-abstraction memory integration
...
All test suites — including the new `test:api` suite — run against the
deterministic **mock** AI and memory providers regardless of your local
`.env`, so nothing here ever requires a live Anthropic or Breeth
credential, and every run is fast, free, and reproducible.
...
**Breeth integration status: implemented, not live-tested.** The
`breethMemoryProvider.js` implementation (unchanged from Part 3) targets
Breeth's documented REST endpoints and request/response shapes exactly as
listed above. It has **not** been exercised against Breeth's live API in
this environment — this sandbox has no network egress to `api.thebreeth.com`,
and the automated test suite deliberately forces `MEMORY_PROVIDER=mock` so
it never requires (or silently depends on) a live credential. Before a real
demo, run one manual write + search against your own Breeth account/key to
confirm end-to-end.
