...
function Report({ profile, feedback, transcript, integrity, onRestart }) {
  ...
  const topicsCovered = [...new Set(exchanges.map((_, i) => `Question ${i + 1}`))];
      ...
      <div id="report-doc" ref={reportRef} className="report-doc">
        ...
        {feedback && (strengthCount > 0 || gapCount > 0) && (
          <section className="coverage-panel">
            <span className="panel-eyebrow">TOPICS COVERED</span>
            ...
            {topicsCovered.length > 0 && (
              <div className="topic-badges">
                {topicsCovered.map((topic) => (
                  <span className="topic-badge" key={topic}>{topic}</span>
                ))}
              </div>
            )}
          </section>
